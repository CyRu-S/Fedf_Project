...
// Run this with: cd backend && node ../clear-database.js

const mongoose = require('mongoose');
const path = require('path');
require('dotenv').config({ path: path.join(__dirname, 'backend', '.env') });

// Define NutritionLog schema inline (to avoid path issues)
...
async function clearDatabase() {
  try {
    ...
    console.log(`📊 Found ${allLogs.length} total nutrition logs in database`);
    ...
    if (remainingLogs.length === 0) {
      console.log('🎉 Database is now clean! All nutrition data has been removed.');
